...
import { normalizeStyle } from "../utils/style-utils.ts";
...
> {
  ...
  // @ts-ignore: mapboxgl.Map instance
  ...
  >(
    ...
    const mapele = Mapbox.savedMaps.pop()!;
    const that = mapele as any as Mapbox<StyleT, CallbacksT, MapT>;
    if (!that) {
      // @ts-ignore:
      return null;
    }
  ...
  _initialize(container: HTMLDivElement) {
    ...
    const viewState = mapOptions.initialViewState || mapOptions.viewState ||
      mapOptions;
  ...
  recycle() {
    ...
    // Assign dynamic type to static type Mapbox
    // @ts-ignore:
  ...
  _updateStyleComponents(
    ...
    if (map.isStyleLoaded()) {
      if (
        "light" in nextProps && map.setLight &&
      ...
      if (
        "fog" in nextProps && map.setFog &&
        !deepEqual(nextProps.fog, currProps.fog)
      ) {
        changed = true;
  ...
  _onPointerEvent = (e: MapMouseEvent<MapT> | MapMouseEvent<MapT>) => {
    ...
    if (cb) {
      if (
        this.props.interactiveLayerIds && e.type !== "mouseover" &&
        e.type !== "mouseout"
      ) {
        e.features = this._hoveredFeatures ||
  ...
  _fireEvent(
    ...
    if (eventType in cameraEvents) {
      if (typeof event === "object") {
        (event as unknown as ViewStateChangeEvent<MapT>).viewState =
...
function getAccessTokenFromEnv(): string | null {
  ...
  try {
    accessToken = accessToken || Deno.env.get("MapboxAccessToken") ||
      Deno.env.get("REACT_APP_MAPBOX_ACCESS_TOKEN") || null;
  } catch {
    // ignore
  }
