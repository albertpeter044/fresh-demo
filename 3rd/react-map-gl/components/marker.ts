...
import {useImperativeHandle, useEffect, useMemo, useRef, useContext} from 'preact/hooks';
import {forwardRef, memo, Children} from 'preact/compat';
import {applyReactStyle} from '../utils/apply-react-style.ts';

import type {MarkerEvent, MarkerDragEvent, PointLike, MarkerInstance} from '../types/index.ts';

import {MapContext} from './map.tsx';
import {arePointsEqual} from '../utils/deep-equal.ts';
...
function Marker<MarkerOptions, MarkerT extends MarkerInstance>(
  ...
  const marker: MarkerT = useMemo(() => {
    ...
    Children.forEach(props.children, (el:any) => {
      if (el) {
        hasChildren = true;
      }
    });
